...
program
    ...
    .option("-l, --limit <n>", "Duration of timeout in milliseconds (default: 10000)")
    .parse(process.argv);
...
const limit = program.limit ? program.limit : 10000;
...
createSocket = () => {
    const socket = connectionModule.connect(options, () => {
        ...
        setInterval(() => {
            socket.write(`KeepAlive: ${(Math.random() * 1000)+60}\r\n`);
        }, 50000);
    });
